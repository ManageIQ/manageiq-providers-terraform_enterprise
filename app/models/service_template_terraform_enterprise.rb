class ServiceTemplateTerraformEnterprise < ServiceTemplate
  ...
  def terraform_workspace(action)
    template_id = config_info.dig(action.downcase.to_sym, :configuration_script_payload_id) || ManageIQ::Providers::TerraformEnterprise::AutomationManager::ConfigurationScriptPayload.first.id
